...
const state = {
  ...
  site: [
    {
      addresseeName: '王大陆',
      addresseePhone: '17657897890',
      addressDetail: '浙江省杭州市余杭区仓前街道656号',
      city: '杭州市',
      province: '浙江省',
      area: '余杭区',
      text: '仓前街道656号'
...
const actions = {
  ...
  delSite ({commit}, params) {
    let result = 'ok'
    if (result === 'ok') {
      commit('delSite', params)
    }
  },
  save ({commit}, params) {
    let result = 'ok'
...
const mutations = {
  ...
  delSite (state, i) {
    Dialog.alert({
      ...
      state.site.splice(i, 1)
    }).catch(() => {
      return false
    })
  },
  save (state, {addresseeName, addresseePhone, addressDetail, city, province, area, text}) {
    if (addresseeName !== '' && addresseePhone !== '' && addressDetail !== '' && city !== '') {
      state.site.push({addresseeName, addresseePhone, addressDetail, city, province, area, text})
      Notify({
        text: '添加地址成功',
        duration: 1000,
        backgroundColor: '#00FF00'
  ...
  editor (state, {list, down}) {
    if (list.addresseeName !== '' && list.addresseePhone !== '' && list.addressDetail !== '' && list.city !== '') {
      state.site.splice(down, 1, list)
    } else {
      Dialog.alert({
        title: '错误',
        message: '请将地址填完'
